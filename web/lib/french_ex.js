...
function phonetiqueController($scope, $mdToast, $mdDialog, NgTableParams, $http, $q) {
    ...
    function pickVoice(langPrefix) {
        var lp = (langPrefix || "fr").toLowerCase();
        return voices.find(v => (v.lang || "").toLowerCase().startsWith(lp)) || null;
    }
    function speak(text, lang) {
        ...
        try {
            ...
            var v = pickVoice(lang || "fr");
            if (v) u.voice = v;
            u.lang = (v && v.lang) || lang || "fr-FR";
            u.rate = 1.0; // adjust if too fast/slow
...
// kick voice load early
    loadVoicesOnce();
// --- IPA visibility: defaults + persistence ---
    $scope.showPron = JSON.parse(localStorage.getItem('showPron') || 'true');          // dictated IPA
    $scope.showTypedPron = JSON.parse(localStorage.getItem('showTypedPron') || 'true'); // typed IPA

    $scope.persistShowPron = function () {
        localStorage.setItem('showPron', JSON.stringify(!!$scope.showPron));
    };
    $scope.persistShowTyped = function () {
        localStorage.setItem('showTypedPron', JSON.stringify(!!$scope.showTypedPron));
    };

// add $timeout to DI array
    phonetiqueController.$inject = ["$scope","$mdToast","$mdDialog","NgTableParams","$http","$q","$timeout"];

// and to the function params
    function phonetiqueController($scope, $mdToast, $mdDialog, NgTableParams, $http, $q, $timeout) {
        // (paste the accent code block anywhere inside)
    }


    $scope.vm = $scope.vm || { ortho: "" };
        ...
        $scope.domainEspeak = "http://localhost:3000/cgi-bin/transcriptioner";
  ...
  if (!$scope.currentEx() || !$scope.currentEx().ortho) return;
  if ($scope.currentEx().ipa) return;
  var w = encodeURIComponent($scope.currentEx().ortho);
  $scope.currentEx().ipa = "";
  $http.get($scope.domainEspeak + "?lang=fr&word=" + w)
    .then(function(response){
      var t = (response.data && response.data.transcriptions && (response.data.transcriptions.fr || response.data.transcriptions["fr-FR"])) || "";
      $scope.currentEx().ipa = t.replace(/\(fr(-FR)?\)/g, "");
    });
};
  ...
  var term = ($scope.vm.ortho || "").trim();
  if (!term) { $scope.typedIpa = ""; return; }
  var w = encodeURIComponent(term);
  $http.get($scope.domainEspeak + "?lang=fr&word=" + w)
    .then(function(response){
      var t = (response.data && response.data.transcriptions && (response.data.transcriptions.fr || response.data.transcriptions["fr-FR"])) || "";
      $scope.typedIpa = t.replace(/\(fr(-FR)?\)/g, "");
    });
};

        $scope.checkOrtho = function(){
  var typed = normalize($scope.vm.ortho);
  var expected = normalize(($scope.currentEx()||{}).ortho);
  if (!expected) return; // no current exercise yet
  if (typed === expected) {
    ...
    $scope.showSimpleToast("error-toast", "The right spelling is: \"" +
             ($scope.currentEx().ortho||'') + "\" instead of \"" + ($scope.vm.ortho||'') + "\"");
    $scope.currentEx().userOrtho = $scope.vm.ortho;
  ...
  $scope.progress[$scope.activityNumber - 1] = $scope.goodEx - $scope.badEx;
        ...
        $scope.pronounce = function () {
            var cur = $scope.currentEx && $scope.currentEx();
            if (!cur || !cur.ortho) return;
            speak(cur.ortho, "fr");
            $scope.loadIpa();
        };

        $scope.typedPronounce = function () {
            if (!$scope.vm || !$scope.vm.ortho) return;
            speak($scope.vm.ortho, "fr");
            $scope.loadIpa();
        };
